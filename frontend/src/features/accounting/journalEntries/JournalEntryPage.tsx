// frontend/src/features/accounting/journalEntries/JournalEntryPage.tsx
...
export const JournalEntryPage = () => {
  ...
  useEffect(() => {
    if (rows.length === 0 && settings?.main_currency_id) {
  ...
  // Plain click selects just this row; Ctrl/Cmd+click toggles it in/out of
  // the current multi-selection. This is click semantics specifically —
  // keyboard-driven selection extension uses handleActiveCellChange instead.
  ...
  // Moves the active cell/row without touching selectedRowIds — used by
  // shift+arrow selection extension so the existing multi-selection isn't
  // collapsed back to a single row.
  ...
  const handleAddRow = useCallback(() => {
    setRows(prev => {
      ...
      // Append rather than only handling the empty-grid case — this now
      // backs the permanent "add row" affordance below the last row too.
  ...
  // First row of the current selection (per spec: footer reflects the first
  // selected row), falling back to the last actively-focused row.
  ...
  const buildPayload = (state: 'draft' | 'booked') => {
    ...
    return {
      ...
      items_attributes: rows.map((row) => ({
        ...
        rate: row.rate ?? null,
        // Credit lines send currency_amount as negative; debit lines as positive.
  ...
  return (
    // height: '100dvh' anchors this page to the viewport so its flex children
    // can size correctly. If this route renders inside a layout that already
    // has a fixed-height shell (e.g. a persistent top app bar), swap this for
    // height: '100%' and make sure every ancestor up to that shell also has
    // an explicit height and overflow: 'hidden' — otherwise a flex child
    // without a definite height will grow to fit its content (all the grid's
    // rows) and the whole page scrolls instead of just the grid.
    <Container
      maxWidth={false}
      sx={{
        height: '100dvh',
    ...
    >
      ...
      {/* This is the only flex child without a fixed size — flex: 1 +
          minHeight: 0 lets it (and only it) claim remaining viewport space
          and hand internal scrolling off to EditableGrid.
          display: 'flex'/'column' is required here too: EditableGrid's
          <Paper> relies on flex: 1 + minHeight: 0 to size itself against
          THIS Box, and flex sizing only applies to an element when its
          parent actually is a flex container. Without this, Paper sized to
          its content instead of the viewport, so the footer either trailed
          a short row list or got clipped by overflow: hidden with a long one. */}
